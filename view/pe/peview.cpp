...
bool PEView::Init()
{
	...
	if (processRtti)
	{
		...
		if (settings && settings->Contains("loader.pe.processRttiVtables"))
			processVFT = settings->Get<bool>("loader.pe.processRttiVtables", this);
		bool processMangled = false;
		if (settings && settings->Contains("loader.pe.processMangledRtti"))
			processMangled = settings->Get<bool>("loader.pe.processMangledRtti", this);
