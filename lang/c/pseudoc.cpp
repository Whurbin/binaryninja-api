...
{
	tokens.SetBracesRequired(BracesAlwaysRequired);
	...
	HighLevelILTokenEmitter& emitter, DisassemblySettings* settings, bool asFullAst, BNOperatorPrecedence precedence)
{
	...
	GetExprText(leftExpr, emitter, settings, asFullAst, precedence);
	emitter.Append(OperationToken, comparison);
	GetExprText(rightExpr, emitter, settings, asFullAst, precedence);
}
	...
	HighLevelILTokenEmitter& emitter, DisassemblySettings* settings, bool asFullAst, BNOperatorPrecedence precedence)
{
	...
	GetExprText(leftExpr, emitter, settings, asFullAst, leftPrecedence);
			...
			rightExpr.size >= leftExpr.size)
	{
		...
		GetHighLevelILFunction()->AddIntegerTextToken(rightExpr, -BNSignExtend(rightExpr.GetConstant<HLIL_CONST>(), rightExpr.size, 8), rightExpr.size, emitter);
		emitter.AppendCloseParen();
		return;
	}
	if ((operand == " - ") && (rightExpr.operation == HLIL_CONST) && lessThanZero(rightExpr.GetConstant<HLIL_CONST>(), rightExpr.size) &&
	...
	bool sizeToken)
{
	...
	if (sizeToken)
		AppendSingleSizeToken(twoOperand.size, TextToken, emitter);
	emitter.AppendOpenParen();
	...
	const HighLevelILInstruction& instr, HighLevelILTokenEmitter& tokens, DisassemblySettings* settings)
{
	...
	AppendSingleSizeToken(instr.size, TextToken, tokens);
	tokens.AppendOpenParen();
...
static void AddNamesForOuterStructureMembers(BinaryView* data, Ref<Type> type, const HighLevelILInstruction& var, vector<string>& nameList)
{
	// Collect names of all chained structure members until we reach a registered type. This allows nested
	// structure members to be renamed from the UI.
	Ref<NamedTypeReference> registeredName = type->GetRegisteredName();
	if (registeredName)
	{
		nameList.insert(nameList.begin(), registeredName->GetName().GetString());
	}
	else
	{
		HighLevelILInstruction inner = var;
		while (true)
		{
			if (inner.operation == HLIL_STRUCT_FIELD)
			{
				Ref<Type> innerType = inner.GetSourceExpr<HLIL_STRUCT_FIELD>().GetType().GetValue();
				size_t innerMemberIndex = inner.GetMemberIndex<HLIL_STRUCT_FIELD>();
				if (innerType && (innerType->GetClass() == NamedTypeReferenceClass))
					innerType = data->GetTypeByRef(innerType->GetNamedTypeReference());
				if (innerType && (innerType->GetClass() == StructureTypeClass) && (innerMemberIndex != BN_INVALID_EXPR)
					&& (innerMemberIndex < innerType->GetStructure()->GetMembers().size()))
				{
					StructureMember innerMember = innerType->GetStructure()->GetMembers()[innerMemberIndex];
					nameList.insert(nameList.begin(), innerMember.name);
					registeredName = innerType->GetRegisteredName();
					if (registeredName)
					{
						nameList.insert(nameList.begin(), registeredName->GetName().GetString());
						break;
					}
					inner = inner.GetSourceExpr<HLIL_STRUCT_FIELD>();
				}
				else
				{
					break;
			...
			else if (inner.operation == HLIL_ARRAY_INDEX)
			{
				inner = inner.GetSourceExpr<HLIL_ARRAY_INDEX>();
			}
			else
			{
				break;
	...
	size_t memberIndex, size_t size, HighLevelILTokenEmitter& tokens, bool deref, bool displayDeref)
{
	...
	switch (fieldDisplayType)
	{
		case FieldDisplayName:
		{
			...
			if (type->GetStructure()->ResolveMemberOrBaseMember(GetFunction()->GetView(), offset, 0,
					[&](NamedTypeReference*, Structure* s, size_t memberIndex, uint64_t structOffset,
						...
						AddNamesForOuterStructureMembers(GetFunction()->GetView(), type, var, nameList);
		...
		case FieldDisplayMemberOffset:
		{
			...
			GetHighLevelILFunction()->AddIntegerTextToken(var, offset, size, tokens);
	...
	DisassemblySettings* settings, bool asFullAst, BNOperatorPrecedence precedence, bool statement)
{
	...
	case HLIL_BLOCK:
		[&]() {
			...
			for (auto i = exprs.begin(); i != exprs.end(); ++i)
			{
				...
				GetExprText(*i, tokens, settings, true, TopLevelOperatorPrecedence, true);
				tokens.NewLine();
			}
		}();
		break;
	...
	case HLIL_FOR:
		[&]() {
			...
			if (asFullAst)
			{
				...
				GetExprText(loopExpr, tokens, settings, asFullAst, TopLevelOperatorPrecedence, true);
	...
	case HLIL_IF:
		[&]() {
			...
			GetExprText(trueExpr, tokens, settings, asFullAst, TopLevelOperatorPrecedence, true);
			tokens.EndScope(scopeType);
			...
			if (falseExpr.operation == HLIL_IF)
			{
				...
				GetExprText(falseExpr, tokens, settings, asFullAst, TopLevelOperatorPrecedence, true);
			}
	...
	case HLIL_WHILE:
		[&]() {
			...
			auto scopeType = HighLevelILFunction::GetExprScopeType(loopExpr);
			tokens.BeginScope(scopeType);
			GetExprText(loopExpr, tokens, settings, true, TopLevelOperatorPrecedence, true);
			tokens.EndScope(scopeType);
			tokens.FinalizeScope();
	...
	case HLIL_SWITCH:
		[&]() {
			...
			{
				GetExprText(caseExpr, tokens, settings, asFullAst, TopLevelOperatorPrecedence, true);
				tokens.NewLine();
			}
			...
			if (defaultExpr.operation != HLIL_NOP && defaultExpr.operation != HLIL_UNREACHABLE)
			{
				...
				GetExprText(defaultExpr, tokens, settings, asFullAst, TopLevelOperatorPrecedence, true);
	...
	case HLIL_CASE:
		[&]() {
			...
			for (size_t index{}; index < valueExprs.size(); index++)
			{
				...
				tokens.Append(TextToken, ":");
				/* TODO: fix this, another hack which prevents semi-colon placement and preserves */
				/* TODO: line information such as the address for the token emitter */
				auto fakeInstruction = instr;
				fakeInstruction.operation = HLIL_IF;
				auto exprGuard = tokens.SetCurrentExpr(fakeInstruction);
	...
	case HLIL_IMPORT:
		[&]() {
			...
			GetHighLevelILFunction()->AddPointerTextToken(instr, constant, tokens, settings, DereferenceNonDataSymbols, precedence);
			if (statement)
				tokens.AppendSemicolon();
		}();
		break;
	...
	case HLIL_VAR_INIT:
		[&]() {
			...
			GetHighLevelILFunction()->AddVarTextToken(destExpr, tokens, instr, instr.size);
			if (variableType)
			{
	...
	case HLIL_VAR_DECLARE:
		[&]() {
			...
			GetHighLevelILFunction()->AddVarTextToken(variable, tokens, instr, instr.size);
			if (variableType)
			{
				for (auto typeToken: postTypeTokens)
				{
	...
	case HLIL_FLOAT_CONST:
		[&]() {
			...
			{
				GetHighLevelILFunction()->AddIntegerTextToken(instr, constant, 8, tokens);
			}
	...
	case HLIL_CONST:
		[&]() {
			GetHighLevelILFunction()->AddConstantTextToken(instr, instr.GetConstant<HLIL_CONST>(), instr.size, tokens, settings, precedence);
			if (statement)
				tokens.AppendSemicolon();
		}();
		break;
	...
	case HLIL_CONST_PTR:
		[&]() {
			GetHighLevelILFunction()->AddPointerTextToken(instr, instr.GetConstant<HLIL_CONST_PTR>(), tokens, settings, AddressOfDataSymbols, precedence);
			if (statement)
				tokens.AppendSemicolon();
		}();
		break;
	...
	case HLIL_ASSIGN:
		[&]() {
			...
			if (isSplit)
			{
				...
				AppendSingleSizeToken(high.size, TextToken, tokens);
				tokens.AppendOpenParen();
				GetExprText(srcExpr, tokens, settings, asFullAst, precedence);
				tokens.AppendCloseParen();
				tokens.NewLine();
...
//				const auto high = destExpr.GetHighExpr<HLIL_SPLIT>();
				...
				AppendSingleSizeToken(low.size, TextToken, tokens);
				tokens.AppendOpenParen();
			}
			...
			if (assignUpdateOperator.has_value() && assignUpdateSource.has_value())
			{
				...
				{
					GetHighLevelILFunction()->AddIntegerTextToken(assignUpdateSource.value(),
						-BNSignExtend(
							assignUpdateSource.value().GetConstant<HLIL_CONST>(), assignUpdateSource.value().size, 8),
						assignUpdateSource.value().size, tokens);
				}
				else
				{
	...
	case HLIL_STRUCT_FIELD:
		[&]() {
			...
			if (fieldDisplayType == FieldDisplayOffset)
			{
				...
				tokens.Append(OperationToken, " + ");
				GetHighLevelILFunction()->AddIntegerTextToken(instr, fieldOffset, instr.size, tokens);
				tokens.AppendCloseParen();
				...
				vector<string> nameList { offsetStr };
				AddNamesForOuterStructureMembers(GetFunction()->GetView(), type, srcExpr, nameList);
			}
	...
	case HLIL_CMP_ULT:
		[&]() {
			...
			if (parens)
				tokens.AppendOpenParen();
			AppendComparison(" < ", instr, tokens, settings, asFullAst, CompareOperatorPrecedence);
			if (parens)
				tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
	...
	case HLIL_MULS_DP:
		[&]() {
			...
			if (parens)
				tokens.AppendOpenParen();
			AppendTwoOperand(" * ", instr, tokens, settings, asFullAst, MultiplyOperatorPrecedence);
			if (parens)
				tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
	...
	case HLIL_DIVS_DP:
		[&]() {
			...
			if (parens)
				tokens.AppendOpenParen();
			AppendTwoOperand(" / ", instr, tokens, settings, asFullAst, DivideOperatorPrecedence);
			if (parens)
				tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
	...
	case HLIL_FLOOR:
		[&]() {
			...
			tokens.AppendOpenParen();
			GetExprText(instr.GetSourceExpr<HLIL_FLOOR>(), tokens, settings);
			tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
		}();
	...
	case HLIL_CEIL:
		[&]() {
			...
			tokens.AppendOpenParen();
			GetExprText(instr.GetSourceExpr<HLIL_CEIL>(), tokens, settings);
			tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
		}();
	...
	case HLIL_FTRUNC:
		[&]() {
			...
			tokens.AppendOpenParen();
			GetExprText(src, tokens, settings);
			tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
		}();
	...
	case HLIL_BOOL_TO_INT:
		[&]() {
			...
			if (parens)
				tokens.AppendOpenParen();
			GetExprText(srcExpr, tokens, settings, asFullAst, TernaryOperatorPrecedence);
			tokens.Append(OperationToken, " ? ");
			GetHighLevelILFunction()->AddIntegerTextToken(instr, 1, 1, tokens);
			tokens.Append(OperationToken, " : ");
			GetHighLevelILFunction()->AddIntegerTextToken(instr, 0, 1, tokens);
			if (parens)
				tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
	...
	case HLIL_TRAP:
		[&]() {
			...
			tokens.AppendOpenParen();
			GetHighLevelILFunction()->AddIntegerTextToken(instr, vector, 8, tokens);
			tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
		}();
	...
	case HLIL_DEREF_FIELD:
		[&]() {
			...
			if (type && (type->GetClass() == StructureTypeClass))
			{
				...
				if (type->GetStructure()->ResolveMemberOrBaseMember(GetFunction()->GetView(), offset, 0,
						[&](NamedTypeReference*, Structure* s, size_t memberIndex, uint64_t structOffset,
							...
							{
								const auto constant = srcExpr.GetConstant<HLIL_CONST_PTR>();
								symbolType = GetHighLevelILFunction()->AddPointerTextToken(
									srcExpr, constant, tokens, settings, DisplaySymbolOnly, precedence);
							}
							else
							{
								GetExprText(srcExpr, tokens, settings, true, MemberAndFunctionOperatorPrecedence);
			...
			if (derefOffset || offset != 0)
			{
				...
				{
					const auto constant = srcExpr.GetConstant<HLIL_CONST_PTR>();
					GetHighLevelILFunction()->AddPointerTextToken(
							srcExpr, constant, tokens, settings, DisplaySymbolOnly, precedence);
				}
				else
				{
					GetExprText(srcExpr, tokens, settings, true, AddOperatorPrecedence);
				}

				tokens.Append(OperationToken, " + ");
				GetHighLevelILFunction()->AddIntegerTextToken(instr, offset, instr.size, tokens);
				tokens.AppendCloseParen();
				if (parens)
					tokens.AppendCloseParen();
			}
	...
	case HLIL_EXTERN_PTR:
		[&]() {
			...
			if (val != 0)
				tokens.AppendOpenParen();
			GetHighLevelILFunction()->AddPointerTextToken(
					instr, instr.GetConstant<HLIL_EXTERN_PTR>(), tokens, settings, AddressOfDataSymbols, precedence);
			if (val != 0)
			{
				char valStr[32];
				if (val >= 0)
